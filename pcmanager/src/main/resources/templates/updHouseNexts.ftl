                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                ...
                                <select class="form-control" style="width: 15%;float: left;margin-right: 15px;" id="s2" >
                                    <option value="" hassubinfo="true" selected>请选择厂家</option>
                                </select>
                                <select class="form-control" style="width: 15%;float: left;margin-right: 20px;" id="s3">
                                    <option value="" hassubinfo="true" selected>请选择型号</option>
                                </select>
                                <button class="btn btn-white" id="addEq" style="float: left" onclick="appendEqInfo()">添加设备</button>
                            </div>
                        </div>
                        ...
                        <form method="post" class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    <input type="radio" value="转账" name="payMethod" >&nbsp;&nbsp;转账
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    function init(){
        sessionStorage.setItem('progress', 'next')
        // 判断是否有出库
        if(flag == 'yet_out'){
            // 有出库 禁止编辑
            ...
            $('#addEq').attr("disabled", "disabled")
            $('[type="radio"]').attr("readonly", true)
            $('.form-control.timepicker').attr("disabled", true)
            $('#pt1').attr("disabled", true)
            $('#receipt').attr("readonly", true)
            $('#agreement').attr("readonly", true)
        }else{
            // 获取设备信息
            getTeamEquipments()
        }
    ...
    function getOrder(){
        $.post('getOrderDetailByHid',{hid:'${hid}'}, function(data){
            if(data.message == 'succ'){
                if (!data.result){
                    return;
                }
                if (!data.result.orderDetail){
                    data.result.orderDetail = []
                }
                for(var i = 0; i < data.result.orderDetail.length; i++){
                    var result = data.result.orderDetail[i];
                    $("#tbody").append(
                        "<tr>" +
                            "<td style='display: none;'>" +
                                "<input name='eid' value=" + "'" + result.eid + "'" + ">" +
                            "</td>" +
                            "<td>" + result.etype + "</td>" +
                            "<td>" + result.efactory + "</td>" +
                            "<td>" + result.emodel + "</td>" +
                            "<td>" + result.eprice + "</td>" +
                            "<td style='width: 18%;'>" +
                                (
                                    (flag == 'yet_out')?
                                        "<input type='text' value='"+result.amount+"' readonly name='amt'/>" :
                                        "<input type='text' value='"+result.amount+"'  name='amt'/>"
                                )+
                            "</td>" +
                            (
                                flag == 'yet_out' ?
                ...
                if(order.payTime){
                    $("#pt1").val(order.payTime)
                }
                $("#receipt").val(order.receipt)
                $("#agreement").val(order.agreement)
                var payType = order.payType == 1 ? '现金' : (order.payType == 2 ? '转账': '支付宝')
                console.log(payType)
                $("input[value='"+payType+"']").attr('checked',true)
                if(order.ostatus){
                    $('form').append('<input type="hidden" name="ostatus" value="'+order.ostatus+'">')
                }
        ...
        if (eid != "") {
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    jQuery("#s2").empty();
                    jQuery("#s3").empty();
                    jQuery("#s2").append("<option hassubinfo='true' value=''>请选择厂家</option>");
                    jQuery("#s3").append("<option hassubinfo='true'  value=''>请选择型号</option>");
                    $("#s1").val('')
                }
            });
        }
    ...
    $("form").submit(function () {
       ...
       /* if (pt1 == "") {
        ...
        $.post('addHouseInfoOrderWorker', {param: JSON.stringify(requestParam)}, function(data){
            if(data.message=='succ'){
                location.href="/checkImg?hid="+requestParam.hid
            }else {
                alert(data.result)
            }
        }, "json")
